...
Run this script after create_private_article.py (which prints the article_id to the terminal, btw).
...
if doi.lower() == 'y':
    ...
    create_private_article.checkOK(doi_res)
...
while True:
    ...
    if proceed.lower() == "y":
        ...
        file_info = create_private_article.initiate_new_upload(base_url, headers, args.article_id, file_to_upload)
        create_private_article.upload_parts(headers, file_info, file_to_upload) 
        up_res = requests.post(file_info['location'], headers=headers)
        create_private_article.checkOK(up_res)
...
if publish.lower() == 'y':
    ...
    create_private_article.checkOK(pub_res)
